...
class SegmentTree():
    def __init__(self, size, three_dim):
        Transition_dtype = np.dtype([('timestep', np.int32), ('state', np.uint8, (84, 84, three_dim)), (
        ...
        self.sum_tree = np.zeros(
            (self.tree_start + self.size,), dtype=np.float32)
    ...
    def _update_nodes(self, indices):
        ...
        self.sum_tree[indices] = np.sum(
            self.sum_tree[children_indices], axis=0)
    ...
    def _retrieve(self, indices, values):
        ...
        # If children indices correspond to leaf nodes, bound rare outliers in case total slightly overshoots
            ...
            children_indices = np.minimum(
                children_indices, self.sum_tree.shape[0] - 1)
...
class ReplayMemory():
    def __init__(self, args, capacity):
        ...
        global blank_trans
        blank_trans = (0, np.zeros((84, 84, three_dim),
                       dtype=np.uint8), 0, 0.0, False)
        ...
        self.n_step_scaling = torch.tensor([self.discount ** i for i in range(
            self.n)], dtype=torch.float32, device=self.device)  # Discount-scaling vector for n-step returns
        # Store transitions in a wrap-around cyclic buffer within a sum tree for querying priorities
        self.transitions = SegmentTree(capacity, three_dim)
    ...
    def _get_transitions(self, idxs):
        transition_idxs = np.arange(-self.history + 1,
                                    self.n + 1) + np.expand_dims(idxs, axis=1)
    ...
    def _get_samples_from_segments(self, batch_size, p_total):
        ...
        while not valid:
            ...
            samples = np.random.uniform(
                0.0, segment_length, [batch_size]) + segment_starts
        ...
        states = torch.tensor(
            all_states[:, :self.history], device=self.device, dtype=torch.float32).div_(255)
        next_states = torch.tensor(
            all_states[:, self.n:self.n + self.history], device=self.device, dtype=torch.float32).div_(255)
        # Discrete actions to be used as index
        actions = torch.tensor(np.copy(
            transitions['action'][:, self.history - 1]), dtype=torch.int64, device=self.device)
        # Calculate truncated n-step discounted returns R^n = Σ_k=0->n-1 (γ^k)R_t+k+1 (note that invalid nth next states have reward 0)
        rewards = torch.tensor(np.copy(
            transitions['reward'][:, self.history - 1:-1]), dtype=torch.float32, device=self.device)
        R = torch.matmul(rewards, self.n_step_scaling)
        # Mask for non-terminal nth next states
        nonterminals = torch.tensor(np.expand_dims(
            transitions['nonterminal'][:, self.history + self.n - 1], axis=1), dtype=torch.float32, device=self.device)
    ...
    def __next__(self):
        if self.current_idx == self.capacity:
            raise StopIteration
        transitions = self.transitions.data[np.arange(
            self.current_idx - self.history + 1, self.current_idx + 1)]
        transitions_firsts = transitions['timestep'] == 0
        blank_mask = np.zeros_like(transitions_firsts, dtype=np.bool_)
